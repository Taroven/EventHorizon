...
ns.talents = {}			-- talent information to query - format = { {tab,position},{tab,position},... }
ns.currentTalents = {}	-- current points spent according to ns.talents - format = ns.currentTalents[i] = currentRank(ns.talents[i])
...
function ns:CheckRequirements()
	--print('CheckTalents')
	table.wipe(self.currentTalents)
	...
	vars.activeTree = 1
	vars.activeSpec = {}
	vars.activeTalentGroup = GetActiveTalentGroup('player')
	vars.currentLevel = UnitLevel('player')
	

	for i,talent in ipairs(self.talents) do
		local rank = select(5,GetTalentInfo(talent[1],talent[2])) or 0
		self.currentTalents[i] = rank
	end
	cc.currentTalents = self.currentTalents
	...
	for i = 1, 3 do
		local points = select(5,GetTalentTabInfo(i))
		vars.activeSpec[i] = points
		if (points > vars.activeSpec[vars.activeTree]) then
			vars.activeTree = i
		end
	end
	if (vars.activeSpec[1] == 0 and vars.activeSpec[2] == 0 and vars.activeSpec[3] == 0) then
		vars.activeTree = 0
	end
	...
	for i,config in ipairs(self.frames.config) do
		...
		if rT then
			local havereq = 0
			for _,req in ipairs(rT) do
				local currentRank
				local tab,index,requiredPoints = unpack(req)
				for k,v in ipairs(self.talents) do	-- check the list of current talents against the bar requirements
					if (v[1] == tab) and (v[2] == index) then
						currentRank = self.currentTalents[k]
						havereq = ((requiredPoints and (currentRank == requiredPoints)) or (not(requiredPoints) and currentRank > 0)) and havereq + 1 or havereq
					end
				end
			end
			if havereq ~= #rT then haveTalentReq = nil end
			--print(GetSpellInfo(config.spellID),tab,index,requiredPoints,havereq,#rT,currentRank,haveTalentReq)
		end
		...
		if haveTalentReq and haveGlyphReq and haveSpecReq and haveLevelReq then
			if frameExists then
				spellframe:Activate()
			else
...
function ns:NewSpell(config)
	...
	if config.requiredTalent then
		for i,talent in ipairs(config.requiredTalent) do
			self:AddCheckedTalent(unpack(talent))
		end
	end
